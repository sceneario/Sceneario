...
class Zend_View_Helper_CustomUrl extends Zend_View_Helper_Url
{
    ...
    public function getAuteurUrl($auteur) {
        $urlParams = array(
            'nomAuteur' => $auteur->getPrenomAuteur() . ' ' . $auteur->getNomAuteur(),
            'idAuteur'  => $auteur->getIdAuteur()
        );
        return $this->customUrl($urlParams,'auteur');
    }
    ...
    public function customUrl(array $urlOptions = array(), $name = null, $reset = false, $encode = true)
    {
        ...
        foreach ($urlOptions as $key => &$urlOption){
            if (!in_array($key, array('isbn', 'id')) && in_array($name, array('couverture', 'planche', 'album', 'serie', 'auteur', 'expo', 'interview', 'galerie'))) {
